...
const sheetVariants = cva(
  'flex flex-col items-stretch fixed z-50 bg-background shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-400',
  {
    variants: {
      side: {
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        {...props}
      >
        {/* Close button - fixed position relative to sheet, always visible */}
        {close && (
          <SheetPrimitive.Close
            data-slot="sheet-close"
            className="cursor-pointer absolute end-4 top-4 z-10 rounded-sm opacity-60 ring-offset-background transition-opacity hover:opacity-100 focus:outline-hidden focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary"
          >
            <X className="h-4 w-4" />
        ...
        <div className="overflow-y-auto p-6 flex-1 gap-4 flex flex-col">
          {children}
        </div>
      </SheetPrimitive.Content>
    </SheetPortal>

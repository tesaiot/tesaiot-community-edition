...
| `SETUP_TOKEN` | generated | One-time token gating the first-run setup wizard (`/setup`). Only honoured while no administrator exists; permanently inert afterwards. Rotated by `make reset-setup`. |
